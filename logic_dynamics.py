...
class DynamicsEngine:
        ...
        for star_type, positions in temp_stars.items():
            if positions:
                results.append({
                    ...
                    "desc": SPECIAL_STARS_DB.get(star_type, "상세 정보가 업데이트 중입니다.")
                })
        return results
    ...
    def scan_disasters(self, branches: list) -> list:
        ...
        for d_type, positions in temp_disasters.items():
            if positions:
                cleanType = d_type.split('(')[0]
                desc_text = DISASTERS_DB.get(d_type, DISASTERS_DB.get(cleanType, "상세 정보가 업데이트 중입니다."))
                results.append({
                    "name": d_type,
                    "position": ", ".join(positions),
                    "desc": desc_text
