...
class MechanicsEngine:
            ...
            for branch in branches:
                ...
                if stem in hidden_stems:
                    ...
                    if hidden_stems[2] == stem: 
                        power_score += 20  # 정기에 뿌리를 내리면 가중치 20
                    else: 
                        power_score += 10  # 초기/중기에 뿌리를 내리면 가중치 10
    ...
    def analyze_gyeokguk_and_yongshin(self, stems: List[str], branches: List[str]) -> Dict[str, str]:
        """격국(사회적 무기) 및 조후 용신(행운의 오행) 도출 알고리즘"""
        day_stem = stems[2]
        month_branch = branches[1]

        # 1. 격국 도출 (월지 십성 기준 특수 예외 처리)
