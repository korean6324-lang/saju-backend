...
class GunghapEngine:
    ...
    def get_samwon_gapja(self, year: int) -> dict:
        ...
        cycle = (offset % 180) // 60
        if cycle == 0: return {"name": "상원갑(上元甲)", "desc": "만물이 태동하고 새롭게 시작되는 양(陽)의 기운이 팽배한 시대"}
        elif cycle == 1: return {"name": "중원갑(中元甲)", "desc": "문명과 제도가 무르익고 기운이 중화되는 만개의 시대"}
        else: return {"name": "하원갑(下元甲)", "desc": "물질명리가 극에 달하고 정신적 성숙이 요구되는 음(陰)의 시대"}
    ...
    def get_gugung_compatibility(self, male_star: int, female_star: int) -> dict:
        """
        [극비 통변] 구궁 팔괘 연산 + 남녀 생극제화(남극녀/여극남) 크로스체크 엔진
        """
        # 1. 5황토성 룰 적용
        ...
        b_data = eight_mansions_db.get(match_result, {"score": 0, "text": "-", "classical": "-", "timing": "-"})

        # 3. 🚨 남녀 생극제화 (오행의 지배/피지배) 정밀 통변
        ...
        gender_dynamics = ""
        if m_elem == f_elem:
            gender_dynamics = "【음양 조화】 [비화] 오행이 동일하여 전우처럼 끈끈합니다. 하지만 자존심이 충돌하면 어느 한쪽도 굽히지 않아 냉전이 극단적으로 길어집니다. 각자의 경제권을 철저히 분리해야 파국을 막습니다."
        elif generates[m_elem] == f_elem:
            gender_dynamics = "【음양 조화】 [남생여] 남편이 아내에게 무한한 에너지를 공급하고 희생합니다. 아내는 남편의 헌신을 딛고 발복하나, 남편의 멘탈과 기운이 지속적으로 빨려 나가니 아내의 명확한 보답이 없으면 파국이 옵니다."
        elif generates[f_elem] == m_elem:
            gender_dynamics = "【음양 조화】 [여생남] 아내가 남편을 완벽하게 내조하여 남편의 잠재력을 무한대로 폭발시킵니다. 가문이 크게 일어서나, 남편이 아내의 뼈 깎는 희생을 당연하게 여기면 아내의 건강이 처참하게 망가집니다."
        elif overcomes[m_elem] == f_elem:
            gender_dynamics = "【음양 조화】 [남극녀] 남편이 아내를 강하게 통제하는 전형적인 가부장적 형국입니다. 아내가 순응하면 집안이 흔들림 없이 돌아가나, 억압이 선을 넘으면 아내의 가슴에 맺힌 화병(원망)이 치명적인 질병으로 발현됩니다."
        elif overcomes[f_elem] == m_elem:
            gender_dynamics = "【음양 조화】 [여극남] 아내가 남편의 기를 완전히 꺾어누르고 가장 노릇을 하는 하극상(下剋上)의 흉상입니다. 남편이 자존심을 완벽히 버리지 못하면 폭력, 외도, 경제적 붕괴 등 끔찍한 파국으로 치달을 수 있는 극위험군 결합입니다."
    ...
    def get_inner_compatibility(self, m_day_branch: str, f_day_branch: str) -> dict:
        """
        침소의 은밀한 속궁합 분석. 충, 원진, 파, 해 등을 가감 없이 팩트폭행 합니다.
        """
        if m_day_branch == f_day_branch:
            return {"relation": "비화(比和)", "status": "권태롭고 편안한 동행", "desc": "【분석】 불타오르는 열정보다는 묵직한 신뢰와 동질감으로 가정을 이끕니다. 피로도는 낮으나 긴장감이 현저히 떨어져 심각한 섹스리스(권태기)에 빠질 위험이 매우 높으니 의식적인 이벤트가 절대적으로 필요합니다."}

        pair = {m_day_branch, f_day_branch}
        
        if pair in self.relations["육합(六合)"]:
            return {"relation": "육합(六合)", "status": "천상의 속궁합 (대길)", "desc": "【분석】 겉보기엔 무심해도 침소에 들면 자석처럼 미친 듯이 이끌립니다. 외도나 파산 등 거대한 흉파가 닥쳐도 결코 서로의 손을 놓지 못하는 무서운 육체적/정서적 결속력을 발휘합니다."}
            
        for samhap in self.relations["삼합(三合)"]:
            if pair.issubset(samhap):
                return {"relation": "삼합(三合)", "status": "의리와 안정의 결합", "desc": "【분석】 서로의 부족한 점을 든든하게 채워줍니다. 뜻이 잘 맞고 속궁합이 무던하여 큰 다툼 없이 안정적으로 재물을 불리고 가정을 번창시킵니다."}
                
        for banghap in self.relations["방합(方合)"]:
            if pair.issubset(banghap):
                return {"relation": "방합(方合)", "status": "형제 같은 강한 결속", "desc": "【분석】 같은 방위의 기운으로 뭉쳐, 부부라기보다는 한 배를 탄 전우처럼 외부의 적에 맞서 가정을 억척스럽게 지켜내는 무서운 결속력입니다."}

        # 흉살(凶殺) 팩트폭행
        if pair in self.relations["충(沖)"]:
            return {"relation": "충(沖)", "status": "치명적 대립과 파괴 (극흉)", "desc": "【분석】 침실(부부궁)이 정면으로 들이받고 박살 나는 최악의 흉상입니다. 애정이 식는 순간 상대를 짓밟으려는 파괴적 증오심으로 돌변합니다. 철저한 각방이나 주말부부로 물리적 거리를 강제 확보하지 않으면 참담한 이별 서류를 쓰게 됩니다."}
            
        if pair in self.relations["원진(怨嗔)"]:
            return {"relation": "원진(怨嗔)", "status": "가학적 애증의 덫 (극흉)", "desc": "【분석】 밤에는 이성을 잃을 정도로 미치도록 끌리다가도, 낮에는 서로를 저주하며 물어뜯는 소모전이 무한 반복됩니다. 죽일 듯이 미워하면서도 헤어지려 하면 질척하게 얽혀서 절대 깔끔하게 떨어지지 못하는 피 말리는 애증의 사슬입니다."}
            
        if pair in self.relations["귀문(鬼門)"]:
            return {"relation": "귀문(鬼門)", "status": "광적 집착과 신경쇠약 (흉)", "desc": "【분석】 성적, 정서적 의존도가 광적으로 치솟으며 상대의 일거수일투족을 통제하려 듭니다. 영혼이 통하는 짜릿함 이면에는 심각한 의처증, 의부증, 편집증으로 발현될 수 있는 초고위험군 폭탄이 숨겨져 있습니다."}

        if pair in self.relations["형(刑)"]:
            return {"relation": "형(刑)", "status": "강압적 통제와 억압 (흉)", "desc": "【분석】 서로의 방식을 강제로 뜯어고치려 듭니다. 침실과 일상에서 끝없는 주도권 싸움이 벌어지며, 한쪽이 자존심을 완벽하게 꺾고 지배에 복종해야만 아슬아슬하게 유지되는 활화산 같은 결합입니다."}

        if pair in self.relations["파(破)"] or pair in self.relations["해(害)"]:
            return {"relation": "파해(破害)", "status": "잠복된 균열과 이간질 (흉)", "desc": "【분석】 겉은 화목해 보이나 가장 밑바닥 신뢰에 치명적인 균열이 잠복해 있습니다. 위기의 순간 각자의 이기심이 발동하여 배신하거나, 외도 및 제3자의 이간질로 침실의 평화가 와장창 깨지기 쉽습니다."}

        return {
            "relation": "무해무덕(無害無德)",
            "status": "이성적이고 건조한 관계",
            "desc": "【분석】 강렬한 끌림도 치명적인 충돌도 없습니다. 육체적 황홀함보다는 철저히 자녀 양육, 경제적 목적을 위해 뭉친 비즈니스 파트너에 가깝습니다. 평온하게 유지되나 정서적 교류가 부족해 뼈저린 고독감이 평생 수반될 수 있습니다."
        }